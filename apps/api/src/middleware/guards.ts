...
export async function ensureVersionAccess(userId: number, versionId: number) {
    ...
    if(isOwner) return { ok: true as const, status: 200, projectId: version.projectId };

    const membership = await prisma.projectMember.findFirst({
        where: { userId, projectId: version.projectId },
    })
    if(!membership) return { ok: false as const, status: 404, msg: 'Not a member' };
